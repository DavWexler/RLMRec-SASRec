"""Train/test all three recommender models across multiple seeds on the Amazon
Reviews 2023 dataset (Books category by default — the richest semantic source).

Mirrors `run_all_yelp.py` but writes everything under `artifacts_amazon/` so
the existing MovieLens-1M and Yelp results are not overwritten. Each
training subprocess is invoked with `--dataset amazon` and an
Amazon-appropriate `--out_dir`.

The raw review and metadata JSONL.gz files are auto-downloaded by
`amazon_data.py` on first run from:

    https://mcauleylab.ucsd.edu/public_datasets/data/amazon_2023/

into `data/amazon/`. No manual download step.

Per-seed runs land in `<out_dir>/seed_<N>/`; aggregate (mean ± std)
artifacts land in `artifacts_amazon/comparison/`. See `run_all.py` for the
...
MODELS = [
    {
        ...
        "out_dir": "artifacts_amazon/sasrec",
    },
    {
        "name": "rlmrec_lightgcn",
        "label": "RLMRec+LightGCN",
        "script": "rlmrec_lightgcn_train.py",
        "out_dir": "artifacts_amazon/rlmrec_lightgcn",
    },
    {
        "name": "rlmrec_sasrec",
        "label": "RLMRec+SASRec",
        "script": "rlmrec_sasrec_train.py",
        "out_dir": "artifacts_amazon/rlmrec_sasrec",
    },
]
...
def run_training(script: str, seed_out_dir: str, seed: int,
    ...
    cmd = [sys.executable, script, "--out_dir", seed_out_dir,
           "--dataset", "amazon", "--seed", str(seed), *extra_args]
    print(f"\n{'=' * 72}\n>>> {' '.join(cmd)}\n{'=' * 72}", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train/test SASRec, RLMRec+LightGCN, RLMRec+SASRec on "
                    "Amazon Reviews 2023 (Books) across multiple seeds.")
    ...
    parser.add_argument("--compare_dir", type=str,
                        default="artifacts_amazon/comparison")
    ...
    parser.add_argument("--amazon_start_year", type=int, default=None)
    parser.add_argument("--amazon_min_inter", type=int, default=None)
    parser.add_argument("--amazon_category", type=str, default=None,
                        help="Amazon Reviews 2023 category. Default in the "
                             "trainer is Books.")
    parser.add_argument("--also_make_overview", action="store_true",
                        help="After comparison, run make_overview.py "
                             "--dataset amazon.")
    ...
    if args.amazon_start_year is not None:
        forward += ["--amazon_start_year", str(args.amazon_start_year)]
    if args.amazon_min_inter is not None:
        forward += ["--amazon_min_inter", str(args.amazon_min_inter)]
    if args.amazon_category is not None:
        forward += ["--amazon_category", args.amazon_category]
    ...
    for model in MODELS:
        ...
        by_model_curves[model["label"]] = agg.read_seed_val_curves(
            out_dir, seeds)
    ...
    agg.plot_comparison_bars_with_err(
        ...
        title="Test metrics — mean ± std across seeds, "
              "Amazon-Books (full-rank)")
    agg.plot_val_curves_with_band(
        by_model_curves, compare_dir / "val_NDCG@10_curves.png",
        title="Validation NDCG@10 — mean ± 1σ across seeds, Amazon-Books")
    ...
    print("\n" + "=" * 72)
    print("Test metrics comparison — mean ± std across seeds (Amazon-Books)")
    print("=" * 72)
    print(summary)
    ...
    if args.also_make_overview:
        ...
        proc = subprocess.run(
            [sys.executable, "make_overview.py", "--dataset", "amazon",
             "--artifacts_root", "artifacts_amazon"])
        if proc.returncode != 0:
            print(f"[warn] make_overview.py exited with code "
                  f"{proc.returncode}")
